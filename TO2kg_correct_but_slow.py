# -*- coding: utf-8 -*-
"""
Created on Thu Sep 26 11:15:25 2024
...
def birth(population, grid_size_lon, grid_size_lat):
    ...
    for individual in population:
        ...
        if random.random() < reproduction_probability:
            ...
            x_index = np.clip(int(offspring_x), 0, grid_size_lon - 1)
            y_index = np.clip(int(offspring_y), 0, grid_size_lat - 1)
            ...
            offspring = {
                'x': x_index,  # Offspring placed at parent's location
                'y': y_index, 
...
for t in range(simulation_time_steps):
    ...
    #Apply the Birth function
    num_births = birth(population,grid_size_lon, grid_size_lat)
